...
$relationshipOpts=array('Null','CS Prospect','CS Prospect - initial contact','One Off Agreement','CS / One-off Prospect','CS Active Member','CS Former Member','CS Passive Member','Bainbridge Hot Prospect','Bainbridge Prospect','Bainbridge Former Client','Bainbridge Client'); 
		...
		if($_GET['statusselect']&&count($_GET['statusselect'])<count($relationshipOpts)) {
			...
			foreach($_GET['statusselect'] as $status) {
				if($status=='Null')
					...
					$status='%'.$status.'%';
				$qRestrictions.="relationshipstatus LIKE '$status' OR ";
			}
			$qRestrictions=substr($qRestrictions,0,strlen($qRestrictions)-3).")";
		}
...
$q="SELECT relationshipstatus,id,name,'N/A','(This row is a PE Firm description search)',website,description,`ebidta-min`,`ebidta-max`,`revenue-min`,`revenue-max`,`investmentsize-min`,`investmentsize-max`,industries,keypersonnel,execswdesc,country,source FROM pefirms4 WHERE ";
		$end="";
...
if(isset($_GET['submit'])) {
	...
	print $s;
?>
<?php if($_GET['inportfcos']||$_GET['inexecdesc']) { ?>
	$('.cellPD').each(function(index, element) {
		for(i in allkeywords)
	        $(this).html($(this).html().replace( new RegExp("("+allkeywords[i]+")","gi"),"<span class='highlight'>$1</span>"));	        /*$(this).html($(this).html().replace( new RegExp(keywords[i],"gi"),"<span class='highlight'>"+keywords[i]+"</span>"));*/
    });
<?php } ?>	
	
<?php if($_GET['inpedesc']) { ?>
	$('.cellD').each(function(index, element) {
		for(i in allkeywords)
	        $(this).html($(this).html().replace( new RegExp("("+allkeywords[i]+")","gi"),"<span class='highlight'>$1</span>"));	        /*$(this).html($(this).html().replace( new RegExp(keywords[i],"gi"),"<span class='highlight'>"+keywords[i]+"</span>"));*/
    });
<?php } ?>
